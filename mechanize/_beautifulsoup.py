    ...
    def _matches(self, chunk, howToMatch):
        ...
        if isList(chunk) and not isinstance(chunk, Tag):
            for tag in chunk:
                if isinstance(
                        tag,
                        NavigableText) and self._matches(
                        tag,
                        howToMatch):
                    return True
            return False
...
class Tag(PageElement):
    ...
    def __str__(self, needUnicode=None, showStructureIndent=None):
        ...
        if showStructureIndent is not None:
        ...
        if self.hidden:
            ...
            s.append(contents)
            if closeTag and showStructureIndent is not None:
                s.append(space)
            s.append(closeTag)
            s = ''.join(s)
        isUnicode = isinstance(s, types.UnicodeType)
    ...
    def renderContents(self, showStructureIndent=None, needUnicode=None):
        ...
        for c in self:
            text = None
            if isinstance(
                    c, NavigableUnicodeString) or isinstance(
                    c, types.UnicodeType):
                text = unicode(c)
        ...
        if currentData:
            if not currentData.strip():
                if '\n' in currentData:
                    ...
                    currentData = ' '
            c = NavigableString
            if isinstance(currentData, types.UnicodeType):
